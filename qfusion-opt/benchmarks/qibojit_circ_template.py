# original source: https://github.com/qiboteam/qibojit-benchmarks/blob/main/benchmarks/circuits/qasm.py (Apache-2.0 license)
# modified by yanjiew1 (https://github.com/wang2346581/Quokka/blob/SC24/optimizer/qasm.py)
...
class AbstractCircuit:
    ...
    def to_qasm(self, theta=None):
        ...
        code = [
            "OPENQASM 2.0;",
            'include "qelib1.inc";',
            f"qreg q[{self.nqubits}];",
            f"creg m[{self.nqubits}];",
        ]
    ...
    def __init__(self, nqubits, shift=""):
        ...
        if len(shift):
            if len(shift) != nqubits:
                raise ValueError(
                    "Shift bitstring of length {} was given for "
                    "circuit of {} qubits."
...
class SupremacyCircuit(AbstractCircuit):
    ...
    def create_cirq_circuit(self):
        ...
        return spc.generate_boixo_2018_supremacy_circuits_v2(
            qubits, self.depth, self.seed
        )
...
class QuantumVolume(AbstractCircuit):
    ...
    def __iter__(self):
        raise NotImplementedError(
            "Iteration is not available for "
    ...
    def __iter__(self):
        qasm = qiskit.qasm2.dumps(self.qiskit_circuit)
        for line in qasm.split("\n"):
            first_word = line.split(" ")[0]
            if first_word not in {"//", "OPENQASM", "include", "qreg"}:
