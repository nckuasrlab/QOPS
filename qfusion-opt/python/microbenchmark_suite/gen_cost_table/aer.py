...
def get_args():
    parser = argparse.ArgumentParser(
        description="Generate cost table Aer",
    ...
    predict_parser.add_argument("input_target_qubit", help="Target qubit", type=int)
...
def main():
    ...
    if args.mode == "gen_table":
        ...
        for gate in gate_list:
            for target_qubit in range(int(input_total_qubit)):
                df_input = pd.DataFrame(
                    [[gate, input_total_qubit, target_qubit]],
                    columns=["gate_type", "total_qubit", "target_qubit"],
                )
        ...
        input_total_qubit = args.input_total_qubit
        input_target_qubit = args.input_target_qubit
        df_input = pd.DataFrame(
            [[input_gate_type, input_total_qubit, input_target_qubit]],
            columns=["gate_type", "total_qubit", "target_qubit"],
        )
        df_input["gate_type"] = pd.Categorical(
            df_input["gate_type"], categories=gate_list
